...
class Experiment(object):
    def __init__(
        ...
        initial=None,
        final_lambda=None,
        lambda_decay_type=None,
    ):
        if initial is None and num_equations is None:
            raise ValueError(
                "At least one of num_equations and initial should be provided."
            )
        ...
        if simulation_type == "cuda":
            ...
            self.sim = simulation.NaiveSimulation(
    ...
    def precompute_lambdas(self, num_iters):
        ...
        if lambdas is None:
            raise ValueError(
                f"Lambda decay type {self.lambda_decay_type} is not supported."
            )
    ...
    def analytical_solution(self):
        ...
        return (
            self.lmbda / (np.sqrt(np.pi) * k ** (3 / 2)) * np.exp(-self.lmbda**2 * k)
        )
