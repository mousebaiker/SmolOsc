...
class ConstantSourceSimulation(FastSimulation):
    ...
    def compute_update(self, concentration):
        ...
        update[2:] = (
            0.5 * self.K_nn(concentration) - (1 + self.lmbda) * concentration * K_n
        )[2:]
        return update
...
class BackwardEulerSimulation(FastSimulation):
    ...
    def simulation_step(self):
        ...
        solution = optimize.root(
            self.implicit_function, initial_approx, method="krylov"
        )
...
class CrankNicolsonSimulation(FastSimulation):
    ...
    def simulation_step(self):
        ...
        initial_approx[np.arange(100, len(self.concentration))] = 0
        rhs = (
            self.concentration + 0.5 * self.compute_update(self.concentration) * self.dt
        )
        solution = optimize.root(
            functools.partial(self.implicit_function, c=rhs),
            initial_approx,
            method="krylov",
        )
        if not solution.success:
