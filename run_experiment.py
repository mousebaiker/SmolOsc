...
def validate_config(config):
    ...
    if num_equations is None and path_to_initial is None:
        print(
            "Please specify number of equations (num_equations key) or initial solution (path_to_initial key)",
            file=sys.stderr,
        )
    result &= num_equations is not None or path_to_initial is not None

    lambda_decay_type = config.get("lambda_decay_type")
    result &= lambda_decay_type is None or validate_required_param(
        config, "final_lambda"
    )
    return result
...
def main():
    ...
    parser.add_argument(
        "yaml_config", help="Config YAML file specifying the parameters of experiment"
    )
    ...
    e.run_experiment(name, config.get("num_iters"), config.get("checkpoint_frequency"))
    ...
    cutoff = 10
    fig, _ = plotting.plot_moments(
        ts[cutoff:], zeroth[cutoff:], first[cutoff:], second[cutoff:]
    )
